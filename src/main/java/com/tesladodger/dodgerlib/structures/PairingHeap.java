...
public class PairingHeap<K extends Comparable<K>, V> {
     ...
     * @return new heap;
     */
    public PairingHeap<K, V> cloneHeap() {
        if (isEmpty()) throw new NullPointerException("The heap is empty.");
        return cloneRecursively(root, new PairingHeap<>(this.type));
    }
     ...
     * @return new heap;
     */
    private PairingHeap<K, V> cloneRecursively (PNode current, PairingHeap<K, V> newHeap) {
        if (current != null) {
    ...
    public static <K extends Comparable<K>, V> PairingHeap
        ...
        if (heap1.type != heap2.type) throw new IllegalArgumentException("Cannot merge heaps of " +
                "different types.");
        PairingHeap<K, V> clone1 = heap1.cloneHeap();
        PairingHeap<K, V> clone2 = heap2.cloneHeap();
    ...
    public V pop () {
        ...
        if (root.child == null) {
            ...
            // Make a list width children of root.
